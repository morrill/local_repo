...
for k in range(len(mm)):  
    ...
    for j in range(len(tn)):   
        for i in range(len(rteo)):
            ...
            eta[k,i,j] = (p[k,i,j]/((Tw - T1)/Rwpluss))*100 # efficiency [%]
...
plt.show()
#%% plots for the value of m which maximizes power for each leg length
...
plt.xscale('log')
plt.show()
#%% Outputs thermal resistance and electrical resitance values at maximum power
