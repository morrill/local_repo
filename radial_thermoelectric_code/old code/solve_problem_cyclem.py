...
mm = np.arange(0.1,4.1,0.1,dtype=float) # ration of electrical load resistance to TE module resistance
...
for k in range(len(mm)):  
    ...
    for j in range(len(tn)):   
        ...
        for i in range(len(rteo)):
            ...
            Relec = (m.log(rteo[i]/rtei)/(2.0*m.pi))*(1.0/(sigma_n*tn[j])+1.0/(sigma_p*tp[j])) + ts*(1/(sigma_c*m.pi*((rtei+100e-6)**2.0 - (rtei)**2.0)) + 1/(sigma_c*m.pi*((rteo[i])**2.0 - (rteo[i]-100e-6)**2.0))) # n- and p-type total electrical resistance
...
plt.subplot(122)
plt.xlabel('Outer radius [$mm$]',fontsize=16)
plt.ylabel('Power Density [$mW/cm^2$]',fontsize=16)
plt.plot((rteo)*1e3,p[int(m_maxp.argmax()),:,int(tn_maxp_mm[m_maxp.argmax()])]*0.1,'r',lw=2)
plt.xscale('log')
#%% older plots
'''
plt.subplot(223)
