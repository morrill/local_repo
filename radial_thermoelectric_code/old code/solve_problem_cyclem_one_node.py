...
tp = (tw-ts)-tn # p-type material length along cylinder
...
for k in range(len(mm)):  
    ...
    for j in range(len(tn)):   
        for i in range(len(rteo)):
            ...
            Relec = ((m.log(rteo[i]/rtei))/(2.0*m.pi*sigma_n*tn[j])) + ((m.log(rteo[i]/rtei))/(2.0*m.pi*sigma_p*tp[j])) + (Rc/((2*m.pi*((rtei+ts)**2.0 - (rtei)**2.0)) + (2*m.pi*(((rteo[i])**2.0) - (rteo[i]-ts)**2.0)))) # n- and p-type total electrical resistance
            [hc, T2] = calc_temps_hc(1,rteo[i],Rte,mm[k])
            p[k,i,j] = (mm[k]/(mm[k]+1.0)**2.0)*(Sp-Sn)**2.0*(Th-T2)**2.0/(Relec*2.0*m.pi*rtei*tw) # power density [W/m^2]
...
Tm = np.mean([Th,T2atmax])
print 'Max power density = %.3f [W/m^2]\nT2 = %.3f [K]\nAverage temp = %.3f [K]\ntn at maxp = %.3f [mm]\nrteo at maxp = %.3f [mm]\nmm at maxp = %.3f' %(p.max(),T2atmax,Tm,maxptn*1e3,maxprteo*1e3,maxpmm)
...
plt.xscale('log')
#%% plots for transposed data to give more accurate curves of of power density vs. leg length
#defines a new array p2
p2 = np.transpose(p, (1, 0, 2))
mm_maxp_rteo = np.zeros(len(rteo))
tn_maxp_rteo = np.zeros(len(rteo))
rteo_maxp = np.zeros(len(rteo))
p2vrteo = np.zeros((len(rteo), len(tn)))

for i in range(len(rteo)):
    [mm_maxp_rteo[i],tn_maxp_rteo[i]] = np.unravel_index(p2[i].argmax(), p2[i].shape) # gets rteo and tn for max p in each mm slice
    rteo_maxp[i] = p2[i].max() # gets max p value from each mm value

for i in range(len(rteo)):
    p2vrteo[i, :] = p2[i, int(round(mm_maxp_rteo[i])),:]
...
#plot contour of power density vs outer radius and n-type material thickness (leglength vs. Pd-rteo)
tnplot = tn/tw
[X,Y] = np.meshgrid(tnplot,(rteo-rtei)*1e3)
size = 10
plt.figure(2,figsize=(2*size,size))
...
plt.ylabel('Leg Length [$mm$]',fontsize=16)
# flips axis because of how p is defined with rows (y axis) as rteo and columns (x axis) as tn
contf = plt.contourf(X,Y, p2vrteo*0.1, levels=np.linspace(0, int(round(maxp*0.1)), 100), extend='both')
cbar = plt.colorbar(contf)
plt.title(r'Power Density [$mW/cm^2$]', fontsize=16)
cbar.set_label('Power Density [$mW/cm^2$]', fontsize=16)
cbar.set_ticks(np.linspace(0, int(maxp*0.1), 10));
...
plt.xlabel('Leg Length [$mm$]',fontsize=16)
plt.ylabel('Power Density [$mW/cm^2$]',fontsize=16)
plt.plot((rteo - rtei)*1e3,p2vrteo[:,int(round(tn_maxp_rteo[rteo_maxp.argmax()]))]*0.1,'r',lw=2)
plt.xscale('log')
...
#electrical values
Relec_te = (m.log(rteo[int(rteo_maxp_mm[m_maxp.argmax()])]/rtei)/(2.0*m.pi))*(1.0/(sigma_n*tn[int(tn_maxp_mm[m_maxp.argmax()])])+1.0/(sigma_p*tp[int(tn_maxp_mm[m_maxp.argmax()])]))
Relec_contact = (ts*(1/(sigma_c*m.pi*((rtei+100e-6)**2.0 - (rtei)**2.0)) + 1/(sigma_c*m.pi*((rteo[int(rteo_maxp_mm[m_maxp.argmax()])])**2.0 - (rteo[int(rteo_maxp_mm[m_maxp.argmax()])]-100e-6)**2.0))))
...
print 'Qh = Qte - 1/2I^2Re + SIT1 = %.3f [W]\nQc = %.3f [W]\nP = %.3f [W]\nPd = %.3f [W/m^2]' %(Qh,Qc,P,Pd)
#%% older plots
'''
plt.subplot(223)
plt.xscale('log')
...
def calc_temps_hc(hc,rteo,Rte,mm):
    ...
    count = 0     
    while True:   
        [T2] = solve_temps(hc,rteo,Rte,mm,Tguess)
        Tguess = (T2)
        #print "T2 = %0.3f" %T2
        hc_new = h_bar(T2,D) 
        t = abs(hc - hc_new)
...
# temperature solver
    ...
    # funtion for equations    
        T2 = p
        return ((Th-T2)/Rte + (Sp-Sn)**2.0*(Th-T2)**2.0/(2.0*Relec*(mm+1.0)**2.0) + (Sp-Sn)**2.0*(Th-T2)*T2/(Relec*(mm+1.0)) - hc*2.0*m.pi*rteo*tw*(T2-Tc))    
    T2 = fsolve(f,Tguess)
    #print "Method 1:\nTw = %0.3f\nT1 = %0.3f\nT2 = %0.3f" %(Tw,T1,T2)
    return T2
