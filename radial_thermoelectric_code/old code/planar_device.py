...
p = np.zeros((len(mm),len(leglength),len(t)), dtype=float) # power
outtemp = np.zeros((len(mm),len(leglength),len(t)), dtype=float) # outside TE material temperature
outconv = np.zeros((len(mm),len(leglength),len(t)), dtype=float)  # cold side convection coefficient
...
for k in range(len(mm)):  
    ...
    for j in range(len(t)):   
        ...
        for i in range(len(leglength)):
            Rte = (kn*t[j]**2.0/leglength[i] + ks*(Ah-t[j]**2.0)/leglength[i] + kp*t[j]**2.0/leglength[i])**-1.0 # TE/spacer total thermal resistance
            Relec = leglength[i]/(sigma_n*t[j]**2.0) + Rc/(4.0*t[j]**2) + leglength[i]/(sigma_p*t[j]**2.0) # contact and n- and p-type total electrical resistance
            [hc, T2] = calc_temps_hc(1,mm[k])
            p[k,i,j] = (mm[k]/(mm[k]+1.0)**2.0)*(Sp-Sn)**2.0*(Th-T2)**2.0/(Relec*Ah) # power density [W/m^2]
            outtemp[k,i,j] = T2 # outer TE material temperatures
            outconv[k,i,j] = hc # convection coefficient
leglength_maxp_mm = np.zeros(len(mm))
t_maxp_mm = np.zeros(len(mm))
m_maxp = np.zeros(len(mm))
for i in range(len(mm)):
    [leglength_maxp_mm[i],t_maxp_mm[i]] = np.unravel_index(p[i].argmax(), p[i].shape) # gets rteo and tn for max p in each mm slice
...
maxpt = t[int(t_maxp_mm[m_maxp.argmax()])]
FFmax = FF[int(t_maxp_mm[m_maxp.argmax()])]
...
T2atmax = outtemp[int(m_maxp.argmax()),int(leglength_maxp_mm[m_maxp.argmax()]),int(t_maxp_mm[m_maxp.argmax()])]
...
plt.ylabel('Power Density [$mW/cm^2$]',fontsize=16)
plt.plot((leglength)*1e3,p[int(m_maxp.argmax()),:,int(t_maxp_mm[m_maxp.argmax()])]*0.1,'r',lw=2)
plt.xscale('log')
...
#%% more output data
#thermal values
Rte = (kn*maxpt**2.0/maxpleglength + ks*(Ah-maxpt**2.0)/maxpleglength + kp*maxpt**2.0/maxpleglength)**-1.0
hc = outconv[int(m_maxp.argmax()),int(leglength_maxp_mm[m_maxp.argmax()]),int(t_maxp_mm[m_maxp.argmax()])]
Rhc = (hc*Ah)**-1.0
print 'Rte = %.3f [W/K]\nhc = %.3f [W/m^2K]\nRhc = %.3f [W/K]' %(Rte,hc,Rhc)

#electrical values
Relec_te = maxpleglength/(sigma_n*maxpt**2.0) +  maxpleglength/(sigma_p*maxpt**2.0)
Relec_contact = (4.0*maxpt**2*3.41e-10)
Relec = Relec_te + Relec_contact
print 'Relec_te = %.3f [mOhms]\nRelec_contact = %.3f [pOhms]\nRelec = %.3f [mOhms]' %(Relec_te*1e3, Relec_contact*1e12,Relec*1e3)
#%% Heat flow model check
#Check model
#Qh = (Th - Twatmax)/Rhh
#Qw = (Twatmax - T1atmax)/Rw
Qte = (Th - T2atmax)/Rte
Half_I2Re = (Sp-Sn)**2.0*(Th-T2atmax)**2.0/(2.0*Relec*(mm[int(m_maxp.argmax())]+1.0)**2.0)
SIT1 = (Sp-Sn)**2.0*(Th-T2atmax)*Th/(Relec*(mm[int(m_maxp.argmax())]+1.0))
SIT2 = (Sp-Sn)**2.0*(Th-T2atmax)*T2atmax/(Relec*(mm[int(m_maxp.argmax())]+1.0))
Qh = Qte - Half_I2Re + SIT1
Qc = (T2atmax - Tc)/Rhc
P = Qh - Qc
Pd = P/(Ah)

print 'Qh = Qte - 1/2I^2Re + SIT1 = %.6f [W]\nQc = %.6f [W]\nP = %.6f [W]\nPd = %.6f [W/m^2]' %(Qh,Qc,P,Pd)
#%% older plots
'''
plt.subplot(223)
plt.xscale('log')
...
# temperature solver
    ...
    # funtion for equations    
        T2 = p
        return ((Th-T2)/Rte + (Sp-Sn)**2.0*(Th-T2)**2.0/(2.0*Relec*(mm+1.0)**2.0) + (Sp-Sn)**2.0*(Th-T2)*T2/(Relec*(mm+1.0)) - hc*Ah*(T2-Tc))    
    T2 = fsolve(f,Tguess)
    #print "Method 1:\nTw = %0.3f\nT1 = %0.3f\nT2 = %0.3f" %(Tw,T1,T2)
    return T2
...
# gets h_bar for iterative problem
    ...
    if condition == 'forced':
        ...
        L = Ah/(2*tw + 2*m.pi*D)
        hC = flat_plate(T_f, T2, D, visc, Prf)*(k/L)
        return hC
...
def flat_plate(T_f, T2, D, visc, Prf):
    L = Ah/(2*tw + 2*m.pi*D)
    Ra = 9.81*(1/T_f)*(T2-Tc)*L**3/(visc**2)*Prf
    if orientation == 'bottom':
        if 1e4 <= Ra < 1e9 and Prf >= 0.7:
            Nu = 0.52*Ra**(1/5)
        else:
