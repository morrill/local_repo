...
# temperature
Th = 373.0         # hot stream tempertaure [K]
...
# geometry
rwi = 6e-3       # inner pipe radius [m]
rtei = 8e-3       # outer pipe/inner TE radius [m]
...
# material properties
mm = 1.53                     # ratio of load resistance to TE resistance
...
#%% solve problem
rteo = np.linspace(200e-6 + rtei, 1, num=1000, dtype=float) # outer TE radius [m] sent in as array of outer radi
tn = np.linspace(0.1*(tw-ts), 0.99*(tw-ts), num=len(rteo), dtype=float)
tp = (tw-ts)-tn
...
for j in range(len(tn)):
    ...
    for i in range(len(rteo)):
        Rte = (m.log(rteo[i]/rtei)/(2.0*m.pi))*(1.0/(kn*tn[j])+1.0/(ks*ts)+1.0/(kp*tp[j])) # TE/spacer total thermal resistance
...
tnplot = tn/tw
[X,Y] = np.meshgrid(tnplot,rteo*1e3)
size = 10
